    ...
    def forward_contexted(
        ...
        for lidx in range(swin_model.num_layers):
            ...
            for bidx, block in enumerate(layer.blocks):
                ...
                x_windows, _ = window_partition(shifted_x, block.window_size)
                dmap_windows, _ = window_partition(dmap_layer, block.window_size)
                dmap_windows = (dmap_windows.mean(dim=(1, 2, 3)) > 0).float()
                # nW*B, window_size, window_size, C
                ...
                if dmap_windows.mean() != 1:
                    ATTN_B_ = int(ATTN_B_ * dmap_windows.mean())
                    x_orig = x_windows
                    x_windows = x_windows[torch.randperm(ATTN_B_)]
                ...
                # merge windows
                x_windows = x_windows.view(-1, block.window_size, block.window_size, Block_C)
                x_windows = window_reverse(x_windows, block.window_size, Hp, Wp)  # B H' W' C

                x_win_shape = x_windows.shape

                # partial projection
                x_sel = x_windows.reshape(-1, Block_C)
                x_sel = F.embedding(dindice_for_embeddings, x_sel)
                
                x_sel = block.attn.proj(x_sel)
                x_sel = block.attn.proj_drop(x_sel)

                fname = f"layer{lidx}_block{bidx}_attn_out"
                if fname in anchor_features:
                    x_cached = anchor_features[fname].clone()
                    x_cached[dindice_for_embeddings] = x_sel
                    x_sel = x_cached
                else:
                    x_cached = x_windows.view(-1, Block_C).clone()
                    x_cached[dindice_for_embeddings] = x_sel
                    x_sel = x_cached
                new_cache_feature[fname] = x_sel.clone()

                x_sel = x_sel.reshape(x_win_shape)
