...
from .modeling.backbone.utils import get_abs_pos, window_partition, window_unpartition, AddDecomposedRelPos, partial_mlp_inference, expand_mask_neighbors
...
class ViTDeT_b_Imagenet_Contexted(ExtendedModule):
    def __init__(self, device="cuda:1"):
        ...
        np.random.seed(42)
        self.VID_COLORS_ARRAY = np.random.randint(256, size=(31, 3)) / 255
        self.VID_LABELS_MAP = {k: v for v, k in enumerate(self.VID_LABELS_LIST)}
    ...
    def load_weight(self, weight_pkl_path='./frcnn_vitdet_final.pth'):
        with open(weight_pkl_path, 'rb') as f:
            weights = torch.load(weight_pkl_path, map_location='cpu')
