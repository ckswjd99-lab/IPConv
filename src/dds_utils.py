...
def evaluate(max_fid, map_dd, map_gt,
    ...
    # For mAP calculation
    all_detections = defaultdict(list)  # class -> list of (score, TP/FP)
    npos = defaultdict(int)            # class -> total GT objects
    ...
    for fid in range(1, max_fid+1):
        ...
        # For mAP: prepare GT usage flags per class
        gt_used = {}
        for i, gt in enumerate(bboxes_gt):
            cls = gt[4]
            gt_used.setdefault(cls, [])
            gt_used[cls].append(False)
            npos[cls] += 1
        
        # Sort detections by confidence (important for AP calc)
        bboxes_dd_sorted = sorted(bboxes_dd, key=lambda x: x[5], reverse=True)
        
        for det in bboxes_dd_sorted:
            found = False
            det_cls = det[4]
            det_score = det[5]
            if det_cls in gt_used:
                for j, gt in enumerate(bboxes_gt):
                    if gt[4] == det_cls and not gt_used[det_cls][j]:
                        if iou(det, gt) >= iou_thresh:
                            gt_used[det_cls][j] = True
                            found = True
                            break
            if found:
                tp += 1
                all_detections[det_cls].append((det_score, 1))
            else:
                fp += 1
                all_detections[det_cls].append((det_score, 0))
        
        for cls, used_list in gt_used.items():
            for used in used_list:
                if not used:
                    fn += 1
        
        # Count overlap (for your 'count' metric)
        for b_gt in bboxes_gt:
            if any(iou(b_gt, b_dd) >= iou_thresh for b_dd in bboxes_dd):
                count += 1
    ...
    f1_list = []
    for tpi, fpi, fni in zip(tp_list, fp_list, fn_list):
        if (2 * tpi + fpi + fni) > 0:
            f1_list.append(round(2.0 * tpi / (2.0 * tpi + fpi + fni), 3))
        else:
            f1_list.append(0.0)
    ...
    for cls, dets in all_detections.items():
        ...
        tps = np.array([d[1] for d in dets_sorted])
        fps = 1 - tps
        tp_cum = np.cumsum(tps)
        fp_cum = np.cumsum(fps)
        recalls = tp_cum / float(npos[cls])
    ...
    return (tp, fp, fn, count,
            round(tp/(tp+fp), 3) if tp+fp>0 else 0,
            round(tp/(tp+fn), 3) if tp+fn>0 else 0,
            round((2.0*tp/(2.0*tp+fp+fn)), 3) if (2.0*tp+fp+fn)>0 else 0,
            f1_list, mAP)

def voc_ap(recalls, precisions):
    """Compute VOC 11-point AP"""
